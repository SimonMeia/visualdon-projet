...
import * as d3 from 'd3'; // Elements DOM de la timeline
...
const sliderDiv = document.querySelector('#slider'); // Elements DOM des periodes

const periodeDeLannee = document.querySelector('#periode-annee');
const periodeInfo = document.querySelector('#periodeInfos'); // Boutons de navigation

const btnNavPeriode = document.querySelector('#nav-periode');
const btnNavAll = document.querySelector('#nav-all'); // Elements DOM du heatmap

let heatmapgraphic = document.querySelector('#heatmap-graphic');
let colorDefinition = document.querySelector('#color-meaning'); // Elements DOM des statistiques
...
let moyenneWeekend = document.querySelector('#moyenneWeekend span.heure'); // En animation ou non

let playing = false; // Tableau contenant un tableau par période

let allPeriodes = createAllPeriodesData(heuresDeCoucher); // Range de couleurs utilisées dans le heatmap

let heatmapColors = d3.scaleThreshold().domain([-240, -120, 0, 120, 240]).range(d3.schemeBlues[6]);
        ...
        for (let date of monthData) {
          if (date.date.getDate() == compteurJour) {
            dataArray.push({
              "col": jourSemaine,
              "ligne": ligne,
              "minutesMinuit": date.minutesMinuit,
              "date": date.date
            });
            added = true;
          }
...
function getUsableDates(datesArray) {
  ...
  for (let date of datesArray) {
    usableDatesArray.push({
      "date": new Date(date.dateFrom),
      "minutesMinuit": date.minutesMinuit
    });
  }
...
function displayHeatMap(chartData, heatMapID) {
  ...
  let margin = {
    ...
    left: 30
      ...
      height = 200 - margin.top - margin.bottom; // Labels of row and columns

  let semaine = [4, 3, 2, 1, 0];
  let jourSemaine = ["Lu", "Ma", "Me", "Je", "Ve", "Sa", "Di"]; // append the svg object to the body of the page

  let idName = "heatmap-" + heatMapID;
  let idNameSelector = "#heatmap-" + heatMapID;
  d3.select("#heatmap-graphic").append("div").attr("id", idName);
  let currentHeatmapGraphic = d3.select(idNameSelector).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // Build X scales and axis:

  let x = d3.scaleBand().range([0, width]).domain(jourSemaine).padding(0.01);
  currentHeatmapGraphic.append("g").attr("transform", "translate(0," + height + ")").call(d3.axisBottom(x)); // Build X scales and axis:

  let y = d3.scaleBand().range([height, 0]).domain(semaine).padding(0.01); // currentHeatmapGraphic.append("g")
  // .call(d3.axisLeft(y));
  // create a tooltip

  const tooltip = d3.select(idNameSelector).append("div").style("opacity", 0).attr("class", "tooltip").style("background-color", "white").style("border", "solid").style("border-width", "2px").style("border-radius", "5px").style("padding", "5px").style("padding", "5px").style("position", "fixed"); // Three function that change the tooltip when user / move / leave a cell

  const mousemove = function (event, d) {
    console.log(d.date.getMonth());
    tooltip.style("opacity", 1).html(addZero(d.date.getDate()) + "." + addZero(d.date.getMonth() + 1) + "." + addZero(d.date.getFullYear()) + " - " + convertMinutes(d.minutesMinuit)).style("left", event.x + 20 + "px").style("top", event.y + 20 + "px");
  };
  ...
  currentHeatmapGraphic.selectAll().data(chartData, function (d) {
    return d.col + ':' + d.ligne;
  }).enter().append("rect").attr("x", function (d) {
    return x(d.col) + 2.5;
  }).attr("y", function (d) {
    return y(d.ligne) - 2.5;
  }).attr("width", x.bandwidth() / 1.1).attr("height", y.bandwidth() / 1.1).style("fill", function (d) {
    return heatmapColors(d.minutesMinuit);
  }).on("mousemove", mousemove).on("mouseleave", mouseleave); // Ajout du mois sous le graphic
...
function displayColorMeaning() {
  ...
  let colorDefinitionD3 = d3.select('#color-meaning').append("svg").attr('width', 135).attr('height', 25); // console.log(heatmapColors.range());
  ...
  for (const color of heatmapColors.domain()) {
    ...
    colorDefinitionD3.append('rect').attr('width', 25).attr('height', 25).attr('x', 25 * counter + 5).attr('y', 0).style('fill', function (d) {
      return heatmapColors(color);
...
let nIntervId; // Gestion des la boucle d'animation
